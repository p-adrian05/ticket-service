...
public class MovieServiceImplTest {
    ...
    public void testCreateMovieShouldCallMovieRepositoryWhenTheInputMovieIsValid() throws MovieAlreadyExistsException {
        // Given
        MovieEntity roomEntity = MovieEntity.builder()
            .id(null)
            .duration(100)
            .title("Test1 title")
            .genreEntity(GENRE_ENTITY1).build();
        Mockito.when(movieRepository.existsMovieEntityByTitle(MOVIE_DTO_1.getTitle())).thenReturn(false);
        Mockito.when(movieRepository.save(roomEntity)).thenReturn(MOVIE_ENTITY_1);
        Mockito.when(genreRepository.findGenreEntityByName(MOVIE_DTO_1.getGenre()))
            .thenReturn(Optional.of(GENRE_ENTITY1));
        // When
        ...
        Mockito.verify(movieRepository).save(roomEntity);
        Mockito.verifyNoMoreInteractions(movieRepository);
    ...
    public void testUpdateMovieShouldCallMovieRepositoryWhenTheInputMovieIsExistsInDatabase()
        throws UnknownMovieException {
        // Given
        Mockito.when(movieRepository.findMovieEntityByTitle(MOVIE_DTO_1.getTitle()))
            .thenReturn(Optional.of(MOVIE_ENTITY_1));
    ...
    public void testUpdateMovieShouldThrowNullPointerExceptionWhenTheMovieGenreIsNull() {
        ...
        MovieDto movieDto = MovieDto.builder()
            ...
            .genre(null)
            .build();
        Mockito.when(movieRepository.findMovieEntityByTitle(movieDto.getTitle()))
            .thenReturn(Optional.of(MOVIE_ENTITY_1));
        // When
        Assertions.assertThrows(NullPointerException.class, () -> underTest.updateMovie(movieDto));
        // Then
