Feature: allows creating pricing components and assigning them to rooms, movies and screenings
  ...
  Background:
    ...
    And the user types the "create movieEntity \"Spirited Away\" aminmation 125" command
    And the user types the "create movieEntity \"Pulp Fiction\" drama 154" command
  ...
  @grade5-requirement
  Scenario: the admin user can create price component and assign it to a movieEntity
    Given the user types the "sign in privileged admin admin" command
    When the user types the "create price component additionalFeeForSatantango 100" command
    And the user types the "attach price component to movieEntity additionalFeeForSatantango Sátántangó" command
    And the user types the "show price for Sátántangó Pedersoli \"2021-03-15 10:45\" 10,5" command
